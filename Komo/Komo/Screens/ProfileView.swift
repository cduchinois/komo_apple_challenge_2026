...
//  type + interests). Permission management is delegated to the system
//  Settings app per Apple's HIG (Privacy > Requesting permission): people
//  view and update their choices in Settings > Privacy, not inside the app.
...
struct ProfileView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 18) {
                // Mascot + companion identity — no more `friend` heading
                // (user's own display name was redundant on this screen) and
                // no companion-customization block (customize flow wasn't
                // effective and has been removed).
