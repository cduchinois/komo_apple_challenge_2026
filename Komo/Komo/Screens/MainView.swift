...
struct MainView: View {
    ...
    /// Measured height of the insight sheet's content, used to drive the
    /// sheet's detent so the popup hugs the card (no dead space below) and
    /// grows automatically for longer insights. Seeded with a reasonable
    /// starting height so the sheet isn't tiny on first present.
    @State private var insightSheetHeight: CGFloat = 320
    @State private var focusDurationSeconds: Int = 180

    /// iPad in iPhone-app compat mode reports the same `.phone` idiom as a
    /// real iPhone but a much smaller top safe-area (~20pt vs ~59pt). We use
    /// that gap as the iPad detector so we can compact the insight card into
    /// a popup on iPad only, freeing enough vertical space for the Feed arc
    /// to sit above the tab bar without scrolling. iPhone keeps the full
    /// inline card since its layout already fits.
    private static let isCompactCompatOnIPad: Bool = {
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: false) {
            VStack(spacing: 0) {
                ...
                // On iPad, the full insight card was eating so much vertical
                // space that the Feed arc slid under the tab bar and users
                // had to scroll to reach it. On iPad the card collapses into
                // a compact chip that opens the full card as a sheet; on
                // iPhone the inline card is fine as-is.
                if Self.isCompactCompatOnIPad {
                    insightChip
                        .padding(.top, 14)
                } else {
                    insightCard
            ...
            // Slightly wider gutters on Home so the arc bubbles + header
            // don't touch the phone-frame edges (matches the reviewer's iPad brief).
            .padding(.horizontal, 36)
            .padding(.top, Theme.Space.screenTop)
            // Reserve breathing room below the Feed bubble so it clears the
            // translucent tab bar on iPad's compact frame.
            .padding(.bottom, 32)
            .frame(maxWidth: .infinity, alignment: .top)
        }
        // ScrollView natively respects both top and bottom safe-area on iPad
        // (unlike a bare VStack, whose safe-area collapses to ~20pt in
        // iPhone-app compat mode), so the header stays below the status bar
        // and the action bubbles stay above the tab bar. Only bounces if
        ...
        .sheet(isPresented: $showInsightSheet) {
            // Full insight card presented as a sheet on iPad. Reuses the same
            // observation / suggestion / action row as the inline card, so
            // taps on Save / Add to calendar / Write note / etc. behave
            // identically — they just close the sheet via their side effects.
            insightSheet
        }
        ...
        .task {
            ...
            #if DEBUG
            if CommandLine.arguments.contains("-KOMO_OPEN_INSIGHT") {
    ...
    private func promptNotificationsAfterOnboarding() async {
        #if DEBUG
        // Skip system permission prompts under the iPad-layout screenshot script.
    ...
    private var header: some View {
        ...
        // Rendered as a pill (dark capsule + white text + soft shadow) so it
        // stays legible over any part of the forest photo — dark-green text
        // over the darker top strip of the background was near-invisible.
        HStack {
    ...
    // MARK: 2b. Compact insight chip (iPad only)
    //
    // Shows a single-line teaser with the sparkle label + start of the
    // observation, ending in a chevron. Tapping it opens the full card as a
    // sheet. Height roughly matches one Reflection action button, freeing
    // enough vertical space that the Feed arc no longer hides under the tab
    // bar on iPad's compact iPhone-app frame.
    ...
    private var insightChip: some View {
        ...
        return Button {
            ...
            HStack(spacing: 10) {
                Text("✨", comment: "Insight chip sparkle icon")
                    .font(.system(size: 15))
                VStack(alignment: .leading, spacing: 2) {
                    Text("Today's insight", comment: "iPad insight chip label")
        ...
        .buttonStyle(.plain)
        .accessibilityLabel("Today's insight: \(r.observation). Double tap to read more.")
    }

    /// Full insight card wrapped in a sheet layout for the iPad popup path.
    /// Reuses the same observation / suggestion / action row as the inline
    /// card so behavior stays identical — the action side effects (save,
    /// remind, write note…) fire the same handlers and the sheet closes
    /// naturally when `showInsightSheet` is toggled off.
    ///
    /// Height auto-fits: the outer VStack measures its intrinsic size and
    /// pushes it into `insightSheetHeight`, which drives a single dynamic
    /// `.height(...)` detent — no dead space under short insights, and the
    /// sheet grows as needed for longer ones.
    private var insightSheet: some View {
        ...
        .background(
            GeometryReader { proxy in
                Color.clear
                    .onAppear { insightSheetHeight = proxy.size.height }
                    .onChange(of: proxy.size.height) { _, newValue in
                        insightSheetHeight = newValue
                    }
            }
        )
