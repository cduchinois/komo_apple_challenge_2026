...
final class AppState {
    ...
    init(data: EnergyDataProviding = MockDataProvider()) {
        ...
        #if DEBUG
        // Test hook: `-KOMO_TEST_SCREEN <KomoScreen.rawValue>` on launch args
        // jumps directly to that screen. Used by the iPad-layout screenshot
        // script so we don't have to click through onboarding by hand.
        // Compiled out of Release builds.
        let args = CommandLine.arguments
        if let idx = args.firstIndex(of: "-KOMO_TEST_SCREEN"),
           idx + 1 < args.count,
           let s = KomoScreen(rawValue: args[idx + 1]) {
